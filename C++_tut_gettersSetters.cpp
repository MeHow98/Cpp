...
int main() //is not able to acces privet part of Movie class anymore
{
   ...
   cout << avengers.getRating(); // thanks to get, set and privet specyfication we were able to prevent putting inside of this value some nonsens like "Dog" above
                                //from now on we are able to put inside rating only valid values of rating, otherwise code will print NR (no rating)
    return 0;
}
